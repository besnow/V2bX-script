...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/besnow/V2bX-script/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/besnow/V2bX-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/besnow/V2bX-script/master/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
...
add_node_config() {
    ...
    core_xray=true
    core_type="1"
    echo -e "${green}节点核心类型固定为：xray${plain}"
...
generate_config_file() {
    ...
    # xray-only fork: always generate a single xray core configuration
    cores_config='[
    {
        "Type": "xray",
        "Log": {
